...
##################################################
N_JOBS             = 10
OUTDIR             = "/pnfs/ldrd/darksectorldrd/persistent/users/{USER}/darksectorsim/test".format(USER = os.getenv("USER")) + str(random.randint(0,10000)) + "/"
...
def main():
  ...
  shutil.copy("opticalsim_job.sh", cache_folder)
  ...
  submit_command = ("jobsub_submit {GRID} -N {NJOBS} " 
      ...
      "-f {TARFILE} "
      "-L {LOGFILE} "
      "file://{CACHE}/opticalsim_job.sh".format(
      GRID       = ("--OS=SL7 -g "
