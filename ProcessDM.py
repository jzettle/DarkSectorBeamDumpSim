...
##################################################
N_JOBS             = 1000
OUTDIR             = "/pnfs/ldrd/darksectorldrd/persistent/users/{USER}/darksectorsim/DM_".format(USER = os.getenv("USER")) + str(random.randint(0,10000)) + "/"
...
def main():
  ...
  shutil.copy("boostedDM.sh", cache_folder)
  ...
  submit_command = ("jobsub_submit {GRID} -N {NJOBS} " 
      ...
      "-f {TARFILE} "
      "-L {LOGFILE} "
      "file://{CACHE}/boostedDM.sh".format(
      GRID       = ("--OS=SL7 -g "
