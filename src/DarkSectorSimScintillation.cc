...
DarkSectorSimScintillation::DarkSectorSimScintillation(const G4String& processName,
                                       G4ProcessType type)
...
DarkSectorSimScintillation::~DarkSectorSimScintillation()
{
...
G4bool DarkSectorSimScintillation::IsApplicable(const G4ParticleDefinition& aParticleType)
{
       return (aParticleType.GetPDGCharge() == 0.0 || 
	       aParticleType.IsShortLived()) ? false : true;
}

void DarkSectorSimScintillation::BuildPhysicsTable(const G4ParticleDefinition&)
{
    if (fFastIntegralTable != nullptr) {
       fFastIntegralTable->clearAndDestroy();
       delete fFastIntegralTable;
...
//
G4VParticleChange*
DarkSectorSimScintillation::AtRestDoIt(const G4Track& aTrack, const G4Step& aStep)

// This routine simply calls the equivalent PostStepDoIt since all the
// necessary information resides in aStep.GetTotalEnergyDeposit()

{
        return DarkSectorSimScintillation::PostStepDoIt(aTrack, aStep);
}
...
//
G4VParticleChange*
DarkSectorSimScintillation::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...
void DarkSectorSimScintillation::BuildThePhysicsTable()
{
...
void DarkSectorSimScintillation::SetScintillationByParticleType(const G4bool scintType)
{
        if (fEmSaturation) {
...
G4double DarkSectorSimScintillation::GetMeanFreePath(const G4Track&,
                                          G4double ,
...
G4double DarkSectorSimScintillation::GetMeanLifeTime(const G4Track&,
                                          G4ForceCondition* condition)
{
        *condition = Forced;
...
G4double DarkSectorSimScintillation::sample_time(G4double tau1, G4double tau2)
{
...
G4double DarkSectorSimScintillation::
...
void DarkSectorSimScintillation::DumpPhysicsTable() const
{
...
G4double DarkSectorSimScintillation::GetQuenchingFactor(G4String aParticleName, G4double aTotalEnergyDeposit)
{
  G4String partType = "";
