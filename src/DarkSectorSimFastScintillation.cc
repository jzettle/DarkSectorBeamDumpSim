...
DarkSectorSimFastScintillation::~DarkSectorSimFastScintillation()
{
...
G4bool DarkSectorSimFastScintillation::IsApplicable(const G4ParticleDefinition& aParticleType)
{
       return (aParticleType.GetPDGCharge() == 0.0 || 
	       aParticleType.IsShortLived()) ? false : true;
}

void DarkSectorSimFastScintillation::BuildPhysicsTable(const G4ParticleDefinition&)
{
    if (fFastIntegralTable != nullptr) {
       fFastIntegralTable->clearAndDestroy();
       delete fFastIntegralTable;
...
//
G4VParticleChange*
DarkSectorSimFastScintillation::AtRestDoIt(const G4Track& aTrack, const G4Step& aStep)

// This routine simply calls the equivalent PostStepDoIt since all the
// necessary information resides in aStep.GetTotalEnergyDeposit()

{
        return DarkSectorSimFastScintillation::PostStepDoIt(aTrack, aStep);
}
...
//
G4VParticleChange*
DarkSectorSimFastScintillation::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...

{
	...
	// Scintillation depends on particle type, energy deposited
	   ...
	   //Adding in additional quenching factor
	   const G4ParticleDefinition *partdef = aTrack.GetParticleDefinition();
	   G4double quench = GetQuenchingFactor(partdef->GetParticleName(),TotalEnergyDeposit);
	   ScintillationYield *= quench;
	   //also overwrite fExcitationRatio to include energy dependent F90 values 
...
void DarkSectorSimFastScintillation::BuildThePhysicsTable()
{
...
void DarkSectorSimFastScintillation::SetScintillationByParticleType(const G4bool scintType)
{
        if (fEmSaturation) {
...
G4double DarkSectorSimFastScintillation::GetMeanFreePath(const G4Track&,
                                          G4double ,
...
G4double DarkSectorSimFastScintillation::GetMeanLifeTime(const G4Track&,
                                          G4ForceCondition* condition)
{
        *condition = Forced;
...
G4double DarkSectorSimFastScintillation::sample_time(G4double tau1, G4double tau2)
{
...
G4double DarkSectorSimFastScintillation::
...
void DarkSectorSimFastScintillation::DumpPhysicsTable() const
{
...
G4double DarkSectorSimFastScintillation::GetQuenchingFactor(G4String aParticleName, G4double aTotalEnergyDeposit)
{
  G4String partType = "";
  G4double quenching = 1.0;
  if(aParticleName == "neutron" || aParticleName == "nuclear" || aParticleName == "nucleus" || aParticleName == "GenericIon" || aParticleName == "Ar40")
    partType = "nr";
  else
    partType = "er";
  if(partType == "nr")
  {
    //Use COHERENT LAr results for quenching implementation, along with energy independent value above 125 keVnr
    G4double E_keVee = aTotalEnergyDeposit*1000.0;
    if(aTotalEnergyDeposit <= 0.125 && aTotalEnergyDeposit >= 0.0)
      quenching = 0.246 + 0.00078*E_keVee;
    else
      quenching = 0.3636;
  }
  //G4cout << "Energy Deposit: " << aTotalEnergyDeposit << G4endl;
  //G4cout << "Quenching: " << quenching << G4endl;
  return quenching;
}

G4double DarkSectorSimFastScintillation::GetComponentRatio(G4String aParticleName, G4double aTotalEnergyDeposit)
{
  G4String partType = "";
